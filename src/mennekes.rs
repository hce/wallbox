...
impl Mennekes {
    pub fn new(host_name: &str, port: u16, polling_interval: std::time::Duration) -> Result<Mennekes> {
                ...
                match tcp::Transport::new_with_cfg(&host_name, cfg) {
                    Ok(mut client) => {
                        while do_run_clone.load(Ordering::Relaxed) {
                            let mut fetch = || {
                                let control_pilot = {
                                    let registers = client.read_holding_registers(104, 1).map_err(|e| std::io::Error::new(std::io::ErrorKind::Other, e))?;
                                ...
                                let (f_i_ac, f_i_dc) = {
                                    let registers = client.read_holding_registers(136, 2).map_err(|e| std::io::Error::new(std::io::ErrorKind::Other, e))?;
                                    let buf = binary::unpack_bytes(&registers);
                                    let mut slice = buf.as_slice();
                                    (slice.read_u16::<byteorder::BE>()?, slice.read_u16::<byteorder::BE>()?)
                                };

                                let (i_l1, i_l2, i_l3, energy, power, u_l1, u_l2, u_l3) = {
                                    let registers = client.read_holding_registers(212, 16).map_err(|e| std::io::Error::new(std::io::ErrorKind::Other, e))?;
                                    let buf = binary::unpack_bytes(&registers);
                                    let mut slice = buf.as_slice();
                                    (slice.read_u32::<byteorder::BE>()?, slice.read_u32::<byteorder::BE>()?, slice.read_u32::<byteorder::BE>()?, slice.read_u32::<byteorder::BE>()?, slice.read_u32::<byteorder::BE>()?, slice.read_u32::<byteorder::BE>()?, slice.read_u32::<byteorder::BE>()?, slice.read_u32::<byteorder::BE>()?)
                                };

                                let (max_allowed_current_signalled, start_time) = {
                                    let registers = client.read_holding_registers(706, 3).map_err(|e| std::io::Error::new(std::io::ErrorKind::Other, e))?;
                                    let buf = binary::unpack_bytes(&registers);
                                    let mut slice = buf.as_slice();
                                    (slice.read_u16::<byteorder::BE>()?, slice.read_u32::<byteorder::BE>()?)
                                };

                                let (ev_required_energy, max_allowed_ev_current, current_energy, charging_duration) = {
                                    let registers = client.read_holding_registers(713, 7).map_err(|e| std::io::Error::new(std::io::ErrorKind::Other, e))?;
                                    let buf = binary::unpack_bytes(&registers);
                                    let mut slice = buf.as_slice();
                                    (slice.read_u32::<byteorder::BE>()?, slice.read_u16::<byteorder::BE>()?, slice.read_u32::<byteorder::BE>()?, slice.read_u32::<byteorder::BE>()?)
                                };

                                let user_id = {
                                    let registers = client.read_holding_registers(720, 10).map_err(|e| std::io::Error::new(std::io::ErrorKind::Other, e))?;
                                    let buf = binary::unpack_bytes(&registers);
                                ...
                                let hems_current  = {
                                    let registers = client.read_holding_registers(1000, 1).map_err(|e| std::io::Error::new(std::io::ErrorKind::Other, e))?;
                                    let buf = binary::unpack_bytes(&registers);
                                    let mut slice = buf.as_slice();
                                    slice.read_u16::<byteorder::BE>()?
                                };
                                    ...
                                    match action {
                                        MennekesAction::SetAmps(amps) => {
                                            if amps.max_hems_current > 16 {
                                                eprintln!("Illegal HEMS current of {} Amps", amps.max_hems_current);
                                            } else {
                                                let hems_current  = (|| {
                                                    let registers = client.read_holding_registers(1000, 1).map_err(|e| std::io::Error::new(std::io::ErrorKind::Other, e))?;
                                                    let buf = binary::unpack_bytes(&registers);
                                                ...
                                                if hems_current.is_err() {
                                                    eprintln!("Unable to read HEMS current: {:?}", hems_current.err());
                                                } else {
                                                    let hems_current = hems_current.unwrap();
                                                    if hems_current == amps.max_hems_current {
                                                        eprintln!("{} == {}, not changing value", hems_current, amps.max_hems_current);
                                                    } else {
                                        ...
                                        MennekesAction::AuthorizeUser(user) => {
                                            let regs = modbus::binary::pack_bytes(user.user_id.as_bytes());
                                            if regs.is_err() {
                                                eprintln!("Unable to pack bytes to authorize user: {:?}", regs.err());
                                            } else {
                                                let regs = regs.unwrap();
                                                match client.write_multiple_registers(1110, &regs) {
                                                    Ok(_) => eprintln!("User authorized."),
                                                    Err(e) => eprintln!("Unable to authorize user! {:?}", e)
                                                }
                                            }
                                        }
                                    }
    ...
    pub fn authorize_user(&self, user_id: String) {
        let action = MennekesAction::AuthorizeUser(AuthorizeUserAction{
            user_id
        });
